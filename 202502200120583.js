    ...
    if (books.length) {
        for (var i = 0; i < books.length; i++) {
            if (i < bookmax) {
				...
				bookhtml += '<div class="col-2 col-md-1  kind">[' + books[i][6] + ']</div>';
				bookhtml += '<div class="col-5 col-md-3  bookname"><a href="' + books[i][0] + '">' + books[i][2] + '</a></div>';
				bookhtml += '<div class="col-5 col-md-4 chap"><a href="' + books[i][1] + '" target="_blank">' + books[i][3] + '</a></div>';
				bookhtml += '<div class="col-md-2 author d-none d-sm-block">' + books[i][4] + '</div>';
				bookhtml += '<div class="col-md-2 time d-none d-sm-block">><a href="javascript:removebook(\'' + books[i][0] + '\')" onclick="return confirm(\'确定要将本书移除吗？\')">移除</a></div>';
				bookhtml += '</div>';
            }
        }
    } else {
