let XMlayEr = {
  ...
  "danMu": {
    ...
    this.paused = true;  // 初始状态设为暂停（等待播放器触发）
    this.showing = true;
    this.height = 15;    // 默认高度（后续会根据字体大小调整）
    ...
    player.on("play", function() {
        danMu.paused = false;
        $(".ec-danMa").addClass("dm-show");
        window.requestAnimationFrame(danMu.frame.bind(danMu)); // 立即触发渲染
    });
    ...
    // 兼容浏览器自动播放策略（Chrome/Safari可能会阻塞初始自动播放）
        ...
        this.apiBackend.read(this.dm_api[i][2], (function(index) {
            return function(error, response) {
                ...
                let danmuData = response ? response.map(item => ({
                    ...
                    color: item[2],
                    author: item[3],
                    text: item[4].includes("777ys") ? "68yy.com 全网影视在线看🎬" : item[4],
                    size: item[7]
                })) : [];
    ...
    $(".dm-send").click(function() {
        const color = $(".content_dmP-2 .on-1").data("color") || "#FFFFFF";
        const type = $(".content_dmP-1 .on-1").data("type") || "right";
        const text = $(".dm-input").val().trim();
        
        if (!text) {
            XMlayEr.tips.msg("弹幕内容不能为空！");
            return;
        }
        danMu.send({ text, color, type });
    });

    // ==================== 5. 初始渲染检查 ====================
    // 如果播放器已经在播放状态（如自动播放成功），立即启动弹幕
    if (!player.paused) {
        this.paused = false;
        window.requestAnimationFrame(this.frame.bind(this));
    }

    console.log("[弹幕系统] 初始化完成，当前状态:", 
        `播放器: ${player.paused ? "暂停" : "播放中"}, ` +
        `弹幕: ${this.showing ? "显示" : "隐藏"}`);
},
