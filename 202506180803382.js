let XMlayEr = {
  ...
  "danMu": {
    ...
    "Init": function () {
  // ================= 初始化核心元素 =================
  const danmuBox = $(".dm-box");
  const danmuContainer = $(".ec-danMa");
  ...
  danmuContainer.toggleClass("dm-show", !danMu.paused);

  // 2. 字体大小计算（原逻辑保留）
  let fontSize = getComputedStyle(this.container)["font-size"];
  this.height = parseFloat(fontSize) + 6;
    ...
    this.apiBackend.read(this.dm_api[i][2], (function(index) {
      return function(error, response) {
        if (error) {
          ...
          danmuData[index] = response ? response.map(function(item) {
            return {
              "time": item[0],
              "type": item[1],
              "color": item[2],
          ...
          XMlayEr.danMu.readAllEndpoints(danmuData);
        }
      };
    })(i));
  }
  ...
  // ================= 事件监听 =================
  // 1. 播放/暂停事件（单次注册避免重复）
  player.off("play").on("play", function() {
    danMu.paused = false;
    danmuContainer.addClass("dm-show");
    danMu.frame();
  });

  player.off("pause").on("pause", function() {
    danMu.paused = true;
    danmuContainer.removeClass("dm-show");
  });

  // 2. 新增：dm-box-cc按钮控制（事件委托确保动态元素可用）
  $(document).off("click", ".dm-box-cc").on("click", ".dm-box-cc", function() {
    const shouldShow = !danmuContainer.hasClass("dm-show");
    danmuContainer.toggleClass("dm-show", shouldShow);
    
    // 强制状态同步
    if (shouldShow && !player.paused) {
      danMu.paused = false;
      danMu.frame();
    } else if (!shouldShow) {
      danMu.paused = true;
  ...
  console.log("弹幕系统初始化完成，当前状态:", 
    danMu.paused ? "暂停" : "播放中", 
    "显示:", danmuContainer.hasClass("dm-show")
  );
},
