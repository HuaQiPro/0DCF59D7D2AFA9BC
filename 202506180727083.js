let XMlayEr = {
  ...
  // 弹幕数据加载（保留原逻辑）
  for (let danmuData = [], i = 0; i < this.dm_api.length; ++i) {
  ...
  // ============= 核心修改部分 =============
  // 1. 初始化时同步播放器暂停状态（修复自动播放状态不同步）
  this.paused = XMlayEr.void.paused;

  // 2. 新增 playing 事件处理（直接显示弹幕，不检查 paused 状态）
  XMlayEr.void.on("playing", function() {
    $(".ec-danMa").addClass("dm-show");
  });

  // 3. 强化 play 事件处理（确保覆盖其他可能的隐藏操作）
  XMlayEr.void.on("play", function () {
    XMlayEr.danMu.paused = false;
    $(".ec-danMa").addClass("dm-show").removeClass("dm-hide"); // 清除隐藏状态
  });
  ...
  switch ("1") {
    ...
    case "2":
      ...
      XMlayEr.void.on("fullscreen", function (isFullscreen) {
        isFullscreen ? danmuBox.show() : danmuBox.hide();
      });
      break;
  }
