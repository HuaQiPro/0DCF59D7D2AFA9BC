let XMlayEr = {
  ...
  "danMu": {
    ...
    this.paused = XMlayEr.void.paused; // 初始状态与播放器同步
    this.showing = true;
    this.height = 15;

    // 强制初始化容器显示状态
    $(".ec-danMa").toggleClass("dm-show", !this.paused);

    // ================ 2. 修复按钮功能 ================
    // 弹幕开关按钮 (dm-box-cc)
    $(document).off("click", ".dm-box-cc").on("click", ".dm-box-cc", function() {
        const isHidden = $(this).hasClass("dm-box-cc2");
        if (isHidden) {
            XMlayEr.danMu.show();
    ...
    // 播放状态变化时同步弹幕显隐
    function syncDanmuState() {
        danMu.paused = player.paused;
        $(".ec-danMa").toggleClass("dm-show", !danMu.paused);
        if (!danMu.paused && danMu.showing) {
            window.requestAnimationFrame(danMu.frame.bind(danMu));
        }
    }

    player.on("play", syncDanmuState);
    player.on("pause", syncDanmuState);
    player.on("playing", syncDanmuState); // 处理自动播放恢复
    player.on("seeked", syncDanmuState);  // 处理跳转后状态

    // 初始状态检查（兼容页面加载时播放器已播放的情况）
    if (!player.paused) syncDanmuState();

    // ================ 4. 弹幕数据加载 ================
    this.api();
    let fontSize = getComputedStyle(this.container).fontSize;
    ...
    for (let i = 0; i < this.dm_api.length; i++) {
        this.apiBackend.read(this.dm_api[i][2], (function(index) {
            return function(error, response) {
                if (error) return;
                let danmuData = response.map(item => ({
                    time: item[0],
                    type: item[1],
                    color: item[2],
                    text: item[4]
                }));
                danMu.dan = danMu.dan.concat(danmuData).sort((a, b) => a.time - b.time);
                
                // 首次数据加载后立即渲染（如果应该显示）
                if (index === 0 && !danMu.paused && danMu.showing) {
                    danMu.frame();
                }
            };
        })(i));
    ...
    console.log("[弹幕系统] 初始化完成", {
        state: `paused: ${this.paused}, showing: ${this.showing}`,
        elements: {
            container: !!this.container,
            btnToggle: $(".dm-box-cc").length,
            btnSettings: $(".dm-box-set").length
        }
    });
},
