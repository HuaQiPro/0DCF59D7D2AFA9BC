let XMlayEr = {
  ...
  "ajax": function (apiData, callback) {
    $.post("https://api_tihuan.huaqi.pro/huaqitv.js", apiData, function (apiResponse) {
      apiResponse.code == 200 ? callback(apiResponse) : XMlayEr.error(apiResponse.msg);
    }, "json").error(function (jqXHR, textStatus, errorThrown) {
      XMlayEr.error("Network error: " + textStatus);
    });
  },
  "XMlayEr": function () {
    $.ajax({
      ...
      "success": function (II1Iii1i) {
        ...
        XMlayEr.ajax({
          ...
          aes_key = apiResponse.aes_key;
          aes_iv = apiResponse.aes_iv;
          XMlayEr.name = apiResponse.name;
          XMlayEr.type = apiResponse.type;
          XMlayEr.vurl = XMlayEr.decrypt(apiResponse.url, aes_key, aes_iv);
          XMlayEr.next = XMlayEr.decrypt(apiResponse.next, aes_key, aes_iv);
          XMlayEr.html = XMlayEr.decrypt(apiResponse.html, aes_key, aes_iv);
          XMlayEr.dmid = apiResponse.dmid;
          XMlayEr.ggdmapi = apiResponse.ggdmapi;
          XMlayEr.load();
        });
      },
  ...
  "cookie": {
    "Set": function (key, value, expireDays = 7, storageType = "1") {
      if (storageType === "1") {
        localStorage.setItem(key, value);
      } else {
        let expireDate = new Date();
        expireDate.setTime(expireDate.getTime() + expireDays * 24 * 60 * 60 * 1000);
        document.cookie = key + "=" + encodeURIComponent(value) + ";path=/;expires=" + expireDate.toUTCString();
      }
    },
    "Get": function (key, storageType = "1") {
      if (storageType === "1") return localStorage.getItem(key); else {
        {
          let cookieMatch = document.cookie.match(new RegExp("(^| )" + key + "=([^;]*)(;|$)"));
          if (cookieMatch != null) {
    ...
    "Del": function (key, storageType = "1") {
      if (storageType === "1") localStorage.removeItem(key); else {
        {
          let expireDate = new Date();
          expireDate.setTime(expireDate.getTime() - 1);
          let cookieValue = this.Get(key, 2);
          cookieValue != null && (document.cookie = key + "=" + encodeURIComponent(cookieValue) + ";path=/;expires=" + expireDate.toUTCString());
        }
      }
    }
  },
  "play": function () {
    ...
    if (videoType === "flv") {
      ...
      playerConfig.customType = {
        "flv": function flvPlayer(videoElement, videoUrl, player) {
          {
    ...
    } else if (videoType === "m3u8" || videoType === "hls") {
      playerConfig.type = "m3u8";
      playerConfig.customType = {
        "m3u8": function hlsPlayer(videoElement, videoUrl, player) {
          if (Hls.isSupported()) {
            {
              const hls = new Hls();
              ...
              player.hls = hls;
              player.once("url", () => hls.destroy());
              player.once("destroy", () => hls.destroy());
            }
          } else {
    ...
    $(document).on("click", ".yxq-vod-list", function () {
      var vodListBox = $(".yxq-listbox");
      if (vodListBox.length > 0) {
        $(".vodlist-of,.r-button").toggle();
        if ($(".yxq-stting").length > 0) {
          vodListBox.removeClass("yxq-stting");
        } else {
          vodListBox.addClass("yxq-stting");
        }
      } else vodListBox.addClass("yxq-stting");
    });
  },
  "load": function () {
    XMlayEr.play();
  ...
  "tips": {
    "msg": function (message, duration) {
      let displayTime = duration || 3000;
      $(".pop-msg").show();
      $(".pop-msg .pop-content").html(message);
      setTimeout(XMlayEr.tips.removeMsg, displayTime);
    },
    "removeMsg": function () {
      $(".pop-msg").hide();
    }
  ...
  "theme": {
    "init": function () {
      let themeColor = "#CC6633",
          themeStyle = ".s-on svg circle,.s-on svg path{fill:" + themeColor + "!important}.t-color{color:" + themeColor + "}.t-bj{background-color:" + themeColor + "}.ec-subtitle p{color: #fff; font-size: 1.6vw;background:#000c;}" + XMlayEr.header.logoCss() + "@media (max-width: 767px){.player-logo{width:100px}}";
      $("<style>").text(themeStyle).appendTo("head");
    }
  },
  "header": {
    "init": function () {
      let logoHtml = "<div class=\"player-logo\"><img src=\"" + XMlayEr.header.logoUrl() + "\" alt=\"logo\"></div>";
      $(".player-header").append(logoHtml);
    },
    "logoUrl": function () {
      return "https://api.huaqi.pro/logo.png";
    },
    "logoCss": function () {
      return ".player-logo{position:absolute;top:10px;left:10px;z-index:100;width:120px;height:auto}.player-logo img{width:100%;height:auto}";
    },
    "marquee": function () {
  ...
  "subtitle": {
    ...
    "text": {
      "init": function (subtitleConfig) {
        ...
        subtitleTrack.kind = "metadata";
        videoElements[0].appendChild(subtitleTrack);
        fetch(subtitleConfig.url).then(response => response.arrayBuffer()).then(buffer => {
          const decoder = new TextDecoder("utf-8");
          const subtitleText = decoder.decode(buffer);
          return this.srtToVtt(subtitleText);
        }).then(vttContent => {
          subtitleTrack.default = true;
          subtitleTrack.kind = "metadata";
          subtitleTrack.src = vttContent.toString();
          subtitleTrack.track.mode = "hidden";
          subtitleTrack.addEventListener("cuechange", this.text.update);
        }).catch(error => {
          console.error("Failed to load subtitle:", error);
          throw error;
        });
      },
      "fixSrt": function (srtContent) {
        return srtContent.replace(/(\d\d:\d\d:\d\d)[,.](\d+)/g, (match, timestamp, milliseconds) => {
          ...
          return timestamp + "," + fixedMs;
        });
      },
      "srtToVtt": function (srtContent) {
        return "WEBVTT \r\n\r\n".concat(
          this.fixSrt(srtContent)
            .replace(/\{\\([ibu])\}/g, "</$1>")
            .replace(/\{\\([ibu])1\}/g, "<$1>")
            .replace(/\{([ibu])\}/g, "<$1>")
            .replace(/\{\/([ibu])\}/g, "</$1>")
  ...
  "updateBattery": function () {
    navigator.getBattery().then(function (battery) {
      ...
      batteryLevel === "10%" ? batteryIndicator.css({
        "width": batteryLevel,
        "background-color": "#ff0000"
      }) : batteryIndicator.css("width", batteryLevel);
      battery.addEventListener("levelchange", function () {
        let newLevel = battery.level * 100 + "%";
        batteryIndicator.css("width", newLevel);
      });
  ...
  "danmaku": {
    "init": function () {
      let danmakuBox = $(".dm-box-cc"),
          danmakuState = XMlayEr.cookie.Get("dm-box-cc"),
          danmakuType1 = XMlayEr.cookie.Get("content_dmP-1"),
          danmakuType2 = XMlayEr.cookie.Get("content_dmP-2"),
          danmakuItems1 = $(".content_dmP-1 .item"),
          danmakuItems2 = $(".content_dmP-2 .item"),
          updateDanmakuState = function (box, type, items) {
            if (type === "1") {
              box.addClass("s-on");
              items.addClass("s-on");
            } else {
              box.removeClass("s-on");
              items.removeClass("s-on");
            }
          };
      updateDanmakuState(danmakuBox, danmakuState, danmakuItems1);
      updateDanmakuState(danmakuBox, danmakuType2, danmakuItems2);
    }
  }
};
