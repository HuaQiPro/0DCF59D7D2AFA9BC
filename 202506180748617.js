let XMlayEr = {
  ...
  "danMu": {
    ...
    "Init": function () {
      let danmuBox = $(".dm-box");
          // 1. 初始化弹幕容器（默认显示，但实际渲染受 paused 控制）
    $(".ec-danMa").addClass("dm-show");
    // 2. 监听播放器事件
    const player = XMlayEr.void;
    const danMu = XMlayEr.danMu;
    // 首次加载时检查播放状态（兼容自动播放）
    if (!player.paused) {
      danMu.paused = false;
      window.requestAnimationFrame(danMu.frame.bind(danMu));
    }
    // 播放事件：启动弹幕渲染
    player.on("play", function() {
      danMu.paused = false;
      $(".ec-danMa").addClass("dm-show");
      danMu.frame(); // 立即触发一帧避免延迟
    });
    // 暂停事件：停止弹幕渲染并隐藏
    ...
    // 额外保险：监听视频实际播放（解决部分浏览器自动播放策略）
    player.video.onplaying = function() {
      if (!danMu.paused) return;
      danMu.paused = false;
      $(".ec-danMa").addClass("dm-show");
      danMu.frame();
    };
    // 调试用日志（完成后可移除）
    console.log("弹幕初始化完成，当前播放状态:", player.paused ? "暂停" : "播放中");
      this.off = true;
      this.api();
      this.container = document.querySelector(".ec-danMa");
      let fontSize = getComputedStyle(document.getElementsByClassName("ec-danMa")[0], null)["font-size"],
        fontSizeNum = fontSize.slice(0, -2);
      this.height = Number(fontSizeNum) + 6;
      for (let danmuData = [], i = 0; i < this.dm_api.length; ++i) this.apiBackend.read(this.dm_api[i][2], function (index) {
        return function (error, response) {
          error ? (error.response, danmuData[index] = []) : (danmuData[index] = response ? response.map(function (item) {
            return {
              "time": item[0],
              "type": item[1],
              "color": item[2],
              "author": item[3],
              "text": item[4].indexOf("777ys") != -1 ? "68yy.com 全网影视在线看🎬" : item[4],
              "size": item[7]
            };
          }) : [], danmuData[index] = danmuData[index], XMlayEr.danMu.readAllEndpoints(danmuData));
        };
      }(i));
      this.content();
      false && $(".dm-input").attr({
      ...
      switch ("1") {
        ...
        case "2":
          danmuBox.hide();
          XMlayEr.void.on("fullscreen", function (isFullscreen) {
            if (isFullscreen) danmuBox.show();else {
              danmuBox.hide();
            }
          });
          break;
